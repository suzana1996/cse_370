...
<div class="col-xs-3">
  ...
  <div class="form-area" style="padding: 10px 10px 110px 10px; ">
    ...
    $query = mysqli_query($conn, "SELECT id, name, price, description FROM food");
    while ($row = mysqli_fetch_array($query)) {
      ...
      <div class="list-group" style="text-align: center;">
        <?php
      echo "<b><a href='editfooditem.php?update= {$row['id']}'>{$row['name']}</a></b>";  
        ?>
      </div>
    ...
    $update = $_GET['update'];
    $query1 = mysqli_query($conn, "SELECT * FROM food WHERE id=$update");
